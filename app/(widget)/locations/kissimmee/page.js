  ...
  return (
    <section className="hero-internal" style={{ backgroundImage: "none", backgroundColor: "var(--teal)", overflow: "hidden" }}>
      ...
      <div className="hero-internal__content">
        <h1>Concrete Coating Services<br/>in Kissimmee, FL</h1>
        <p>High-performance coatings built to handle heavy use, Florida weather, and the demands of busy homes and rental properties.</p>
      </div>
    </section>
  );
}
    ...
    <section className="details-section details-section--white" style={{ background: "#fff", padding: "80px 24px" }}>
      <div className="details-section__grid" style={{ maxWidth: "1200px", margin: "0 auto", gap: "60px", gridTemplateColumns: "1fr 1fr", alignItems: "center", display: "grid" }}>
        <div className="details-section__content">
          ...
          <h2 style={{ fontSize: "36px", marginBottom: "20px", color: "var(--teal)" }}>Your Trusted Concrete Coating Experts in Kissimmee</h2>
          <p style={{ marginBottom: "20px", color: "#555", lineHeight: "1.6" }}>We provide professional concrete coating services throughout Kissimmee, helping homeowners and property owners upgrade and protect their spaces. From garage floors to pool decks and driveways, our coatings are built to withstand constant use, moisture, and Florida's harsh sun.</p>
          <p style={{ marginBottom: "30px", color: "#555", lineHeight: "1.6" }}>Kissimmee homes often see higher foot traffic and frequent turnover, which means surfaces take more wear. That's why we focus on durable, easy-to-maintain coatings that not only look great but hold up over time without peeling, cracking, or fading.</p>
        ...
        <div className="details-section__image-wrap">
          ...
          <a href="https://www.google.com/maps/search/?api=1&query=Google&query_place_id=ChIJ5wsVNxqE3YgRDcL9EZfN55Q" target="_blank" rel="noopener noreferrer">
             <iframe 
                src="https://maps.google.com/maps?q=Kissimmee,+FL&t=&z=13&ie=UTF8&iwloc=&output=embed" 
                width="100%" 
                height="350" 
...
export default function KissimmeePage() {
  return (
    <main>
      ...
      <ReviewsSection className="reviews reviews--white" city="Kissimmee" />
      <CTASection />
      <Footer />
    </main>
  );
